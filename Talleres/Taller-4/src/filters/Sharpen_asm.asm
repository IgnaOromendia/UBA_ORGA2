...
Sharpen_asm:
    ...
    movdqu xmm6, [black]    ; crear marco
    movdqu xmm7, [take_bor] ; xmm7 -> [ff,0,0,ff] (en realidad son mas ff pero se entiende que hace) ignorar los medio
    ...
    loop_sharpen:
        ...
        loop_line:
            ...
            pmovzxbw xmm0, xmm0         ; extendemos de 8 a 16
            psrldq xmm1, 8              ; movemos de derecha a izq <- [px2,px3,0,0]
            pmovzxbw xmm1, xmm1         ; extendemos

            ; repetimos para los 4 px del medio
            ...
            ; |     xmm2    |       xmm3      |
            ; |    p4, p5   |      p6, p7     |
            ; estos de que deberian estar ya no importan xq estan arriba
            ...
            psubsw xmm4, xmm0   ; xmm4-r(xmm0) p5 - (p1 + p9)
            psubsw xmm5, xmm1   ; xmm5-l(xmm1) p6 - (p2 + p10)

            psrldq xmm4, 8      ; desplazamos xmm4 hacia la izq
            pslldq xmm5, 8      ; desplazamos xmm5 hacia la derecha

            psubsw xmm4, xmm0   ; xmm4-l(xmm0) p5 - (p0 + p8)
            psubsw xmm5, xmm1   ; xmm5-r(xmm1) p6 - (p2 + p10)

            psubsw xmm4, xmm2   ; xmm4-l(xmm2) p5 - p4
            psubsw xmm5, xmm3   ; xmm5-r(xmm3) p5 - p7

            psubsw xmm4, xmm1   ; xmm4-l(xmm1) p5 - (p2 + p10)
            psubsw xmm5, xmm0   ; xmm4-r(xmm0) p5 - (p1 + p9)

            psubsw xmm4, xmm3   ; xmm4-l(xmm3) p5 - p6
            psubsw xmm5, xmm2   ; xmm4-r(xmm2) p6 - p5

            ; volvemos a acomadar
            pslldq xmm4, 8      ; desplazamos xmm4 hacia la derecha
            psrldq xmm5, 8      ; desplazamos xmm5 hacia la izq

            packuswb xmm4, xmm5         ; unimos xmm4 y xmm5 y convertimos los valores de 16 en 8bits con sat [0,sh1,sh2,0]
            sub r11, r8                 ; volves a apuntar donde estabas
